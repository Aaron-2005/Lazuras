...
from tileset import SOLID_TILES, GHOST_TILES, T_PL_HAZ, T_LAZ_FL, T_LAZ_WL
...
class Level:
    def __init__(self, tile_data, objects, tileset):
        ...
        for obj in objects:
            ...
            if ot == 'spawn':
                ...
                self.pads.append({
                    'rect': pygame.Rect(ox, oy, TILE, TILE),
                    'id': obj['id'],
                    'touched': False
                })
    ...
    def _rebuild_gates(self):
        self.gate_solid = [
            g.rect() for g in self.gates.values()
    ...
    def _activate_targets(self, targets, particles, sound_mgr=None):
        for tid in targets:
            if tid in self.gates:
                ...
                burst(
                    particles,
                    self.gates[tid].x + TILE // 2,
                    self.gates[tid].base_y,
    ...
    def _deactivate_targets(self, targets):
        for tid in targets:
            if tid in self.gates:
                if not any(p.active for p in self.plates if tid in p.targets) and \
                   not any(lv.active for lv in self.levers if tid in lv.targets):
    ...
    def update(self, player, cam, interact_pressed=False, sound_mgr=None):
        ...
        for plate in self.plates:
            ...
            if plate.active and not was:
                burst(self.particles, plate.x + TILE // 2, plate.y + TILE // 2,
                      C_GREEN, 14, 2.5, life=30)
        ...
        if not player.dead:
            ...
            # Exit
            if not player.dead and self.exit_rect and player.rect().colliderect(self.exit_rect):
    ...
    def _draw_lava_floor(self, surf, sx, sy, gt):
        # Base dark rock border
        pygame.draw.rect(surf, (40, 18, 10), (sx, sy, TILE, TILE))
        pygame.draw.rect(surf, (70, 30, 12), (sx, sy + 2, TILE, TILE - 2))

        # Main lava body
        pygame.draw.rect(surf, (190, 40, 10), (sx + 1, sy + 4, TILE - 2, TILE - 5))
        pygame.draw.rect(surf, (235, 85, 10), (sx + 2, sy + 8, TILE - 4, TILE - 10))

        # Animated wave line
        pts = []
        for i in range(0, TILE + 1, 4):
            wave_y = sy + 9 + int(math.sin(gt * 0.18 + i * 0.35 + sx * 0.03) * 2)
            pts.append((sx + i, wave_y))
        pts += [(sx + TILE, sy + TILE), (sx, sy + TILE)]
        pygame.draw.polygon(surf, (255, 120, 20), pts)

        # Bright hot streaks
        for i in range(3):
            ox = (gt * (1.2 + i * 0.25) + i * 9 + sx * 0.2) % TILE
            pygame.draw.ellipse(
                surf,
                (255, 200, 70),
                (sx + int(ox) - 3, sy + 10 + (i * 5) % 10, 8, 4)
            )

        # Bubble pops
        for i in range(2):
            bx = sx + 6 + ((gt * (0.8 + i * 0.2) + i * 11) % 18)
            by = sy + 14 + int(math.sin(gt * 0.12 + i) * 3)
            pygame.draw.circle(surf, (255, 170, 60), (int(bx), int(by)), 2)

        # Glow
        glow = pygame.Surface((TILE, TILE), pygame.SRCALPHA)
        pygame.draw.rect(glow, (255, 90, 20, 45), (0, 6, TILE, TILE - 6))
        surf.blit(glow, (sx, sy))

    def _draw_lava_wall(self, surf, sx, sy, gt):
        # Dark outer stone
        pygame.draw.rect(surf, (38, 16, 10), (sx, sy, TILE, TILE))

        # Vertical lava channel
        pygame.draw.rect(surf, (120, 22, 8), (sx + 6, sy, TILE - 12, TILE))
        pygame.draw.rect(surf, (220, 70, 10), (sx + 8, sy, TILE - 16, TILE))

        # Animated inner stream
        for y in range(0, TILE, 6):
            wobble = int(math.sin(gt * 0.18 + y * 0.45 + sy * 0.04) * 2)
            pygame.draw.ellipse(
                surf,
                (255, 155, 40),
                (sx + 9 + wobble, sy + y, TILE - 18, 8)
            )

        # Bright center
        pygame.draw.rect(surf, (255, 210, 90), (sx + 13, sy + 2, TILE - 26, TILE - 4))

        # Glow
        glow = pygame.Surface((TILE, TILE), pygame.SRCALPHA)
        pygame.draw.rect(glow, (255, 100, 20, 40), (5, 0, TILE - 10, TILE))
        surf.blit(glow, (sx, sy))

    # ── DRAW ─────────────────────────────────────────────
    def draw(self, surf, cam, far_layer, mid_layer, player, gt, fsm):
        px = int(cam.x * 0.08) % SW
        surf.blit(far_layer, (-px, 0))
        surf.blit(far_layer, (SW - px, 0))

        px2 = int(cam.x * 0.18) % SW
        surf.blit(mid_layer, (-px2, 0))
        surf.blit(mid_layer, (SW - px2, 0))

        bx, by = cam.apply(0, 0)
        surf.blit(self.bg, (bx, by))
        ...
        for (col, row), tid in self.tile_map.items():
            if tid not in GHOST_TILES:
                continue
            sx, sy = cam.apply(col * TILE, row * TILE)
            if sx < -TILE or sx > SW or sy < -TILE or sy > SH:
                continue
        ...
        for (col, row), tid in self.tile_map.items():
            ...
            if tid == T_LAZ_FL:
                self._draw_lava_floor(surf, sx, sy, gt)
                continue
            elif tid == T_LAZ_WL:
                self._draw_lava_wall(surf, sx, sy, gt)
                continue
